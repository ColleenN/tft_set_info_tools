...
`Augment(dataclass, frozen)` - One augment, normalized to `name`, `api_name`, `tier: AugmentTier | None`, `effects: dict`. Built by `TFTSetData.get_seed_augments()`, which classifies `tier` polymorphically via `SetDataSchema.get_augment_tier()` -- unlike `Item` below, this works the same for every registered schema.
